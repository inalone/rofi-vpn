...
determine_connection_list() {
	...
	active_connection=$(nmcli --mode tabular --terse connection show --active | grep wireguard | cut -d ':' -f1) || true
	...
	available_connections=$(nmcli --mode tabular --terse connection show | grep wireguard | cut -d ":" -f1) || true
